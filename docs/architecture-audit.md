...
- `CONFIG_VALIDATION_FAILED`: emitted by `config/validator.py:477`, consumed only by `SecurityService`'s audit log — which had no live reader until §1.2 folded the log into the support bundle. Now readable via Capture Logs. `_config_degraded`, set alongside it, is still **written and never read**: "degraded mode" has no behaviour.
