...
> · 19 OPEN · 1 BLOCKED on a product decision. Seven gate blind spots were the
> durable half; see the gate table.
...
| 4b | No frontend renders a toast action | **BLOCKED on 37** | Re-derived 2026-08-26 and the framing was wrong in *both* directions. `retry-sync` is **live end to end**, not a lone special case: `CloudSaveService._emit_save_conflict` emits it with real local/remote snapshots and `CloudSaveConflictModal` renders it, so a blocked upload already reaches the user. The verbs that *are* unreachable as toast actions (`auth`, `refresh-library`, `refresh-all-libraries`, `open-save-folder`, `show-logs`) had exactly one would-be producer — the dead `cloud_failure.py`. **So a generic renderer would today have nothing to render**, and building one is the half-present shape the audit warns about. This is not a renderer job; it is item **37**'s product decision. `show-logs`/`open-save-folder` additionally need modals that do not exist, and Capture Logs already delivers the same files. |
