...
| `find_upc_in` + 3 store-side lookups ×4 | Remaining members of that group. Shape is "a named child of `drive_c`, if it exists"; a shared `in_drive_c(prefix, rel, must_be=...)` would fold them, but Ubisoft's carries a recorded incident (its several `upc.exe` lookups had never been ported to `resolve_drive_c`, so auth/install silently missed a real binary) — worth reading before merging. |
